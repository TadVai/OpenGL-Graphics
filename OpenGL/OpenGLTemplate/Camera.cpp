...
void CCamera::Set(glm::vec3 &position, glm::vec3 const &viewpoint, glm::vec3 const &upVector)
{
	m_position = position;
	m_view = viewpoint;
	m_upVector = upVector;
...
void CCamera::TranslateByKeyboard(double dt)
{
	...
	if (GetKeyState('E') & 0x80) {
		RotateViewPoint(0.745,glm::vec3(0,1,0));
	}
	// Look left
	if (GetKeyState('Q') & 0x80) {
		RotateViewPoint(0.25, glm::vec3(0, 1, 0));
	}
	// 3rd person view
	if (GetKeyState('V') & 0x80) {
		m_position.y = m_position.y + 30.f;
	}
	...
	if (GetKeyState('X') & 0x80) {
		m_position.y = m_position.y + (1.5f*sin(dt));
		m_position.x = m_position.x + (2*sin(dt));
	}

	if (GetKeyState('Z') & 0x80) {
		m_position.y = abs(m_position.y + (sin(0.001*dt)));
